    ...
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route("admin.transactions.create") }}">
                {{ trans('global.add') }} {{ trans('cruds.transactions.title_singular') }}
            </a>
        </div>
    </div>
@endcan
<div class="card">
    <div class="card-header">
        {{ trans('cruds.transactions.title_singular') }} {{ trans('global.list') }}
    </div>
            ...
            <table class=" table table-bordered table-striped table-hover datatable datatable-User">
                <thead>
                    <tr>
                        ...
                        <th>
                            {{ trans('cruds.transactions.fields.name') }}
                        </th>
                        <th>
                            {{ trans('cruds.transactions.fields.sample') }}
                        </th>
                        <th>
                            {{ trans('cruds.transactions.fields.description') }}
                        </th>
                        <th>
                            {{ trans('cruds.transactions.fields.status') }}
                        </th>
                        <th>
                            &nbsp;
                        </th>
                ...
                <tbody>
                    @foreach($transactions as $key => $transaction)
                        <tr data-entry-id="{{ $transaction->id }}">
                            ...
                            <td>
                                <img src="{{ $transaction->sample ?? '' }}" width="50px" height="50px"  />
                            </td>
                            <td>
                                {{ $transaction->description ?? '' }}
                            </td>
                            <td>
                                {!! $transaction->status == 1 ? '<span class="badge badge-success">Active</span>' : '<span class="badge badge-danger">Inactive</span>' !!}
                            </td>
                            <td>
                                @can('user_show')
